...
# UDP_RESV_IP="192.168.43.220" #このPCのIPアドレス
# UDP_RESV_PORT=22222 #受信ポート
UDP_RESV_IP="172.24.65.64" #このPCのIPアドレス
UDP_RESV_PORT=22222 #受信ポート

UDP_SEND_IP="192.168.43.82" #送信先のESP32のIPアドレス
UDP_SEND_PORT=22224 #送信ポート

MSG_SIZE = 90 #Meridim配列の長さ(デフォルトは90)
MSG_BUFF = MSG_SIZE * 2 #Meridim配列のバイト長さ

STEP = 75  #1フレームあたりに増加させる制御処理用の数値,サインカーブを何分割するか
#STEP = 0.066 #1フレームあたりに増加させる制御処理用の数値
...
def meridian_loop():
    ...
    while (True):
        ...
        with closing(sock):
            while True:
                ...
                if flag_demo_action: #
                    ...
                    s_meridim_motion[35] = int(np.sin(x*2)*2000) -500 #左股ピッチ
                    ...
                    s_meridim_motion[65] = -int(np.sin(x*2)*2000) -500 #右股ピッチ
        ...
        # データを送信Meridim配列に格納
                ...
                message3="ERROR RATE ESP-PC:"+str("{:.2%}".format(error_count_esp_to_pc/loop_count+0.0001))+\
                    " PC-ESP:"+str("{:.2%}".format(error_count_pc_to_esp/loop_count+0.0001))+" ESP-TSY:"+str("{:.2%}".format(error_count_esp_to_tsy/loop_count+0.0001))+\
                    " TsySKIP:"+str("{:.2%}".format(error_count_tsy_skip/loop_count+0.0001))+" ESPSKIP:"+str("{:.2%}".format(error_count_esp_skip/loop_count+0.0001))
        ...
        while dpg.is_dearpygui_running():
            ...
            if flag_ros1_pub:#ROS送信:joint_statesのpublishを実施
                if flag_ros1==0:
                    ...
                    flag_ros1=1
                joint_pub = rospy.Publisher('/khr3_001/joint_group_position_controller/command', Float64MultiArray, queue_size=10)
